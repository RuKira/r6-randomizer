...
export function rollOperatorsForRole({
    ...
    while (result.length < 6) {
        const pool = cleanedList.filter(op => op.enabled && (allowDupes || !usedNames.has(op.name)));
        ...
        cleanedList = cleanedList.map(o => o.name === op.name ? {...o, weight: Math.max(1, o.weight - 5)} : o);
    }
...
export const rerollOperator = ({
    ...
    const pool = list.filter(op => op.enabled && (allowDupes || !usedNames.has(op.name)));
    ...
    const updatedChosen = chosen.map(op => op.uid === uid ? {...newOp, uid: newUid} : op);
